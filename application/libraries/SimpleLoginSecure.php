...
class SimpleLoginSecure
{
	 ...
	 * @return	bool
	 */
	function create($user_email = '', $user_pass = '', $auto_login = true, $user_username = null)
	{
	...
	function login($user_email = '', $user_pass = '') 
	{
		...
		if ($query->num_rows() > 0) 
		{
			...
			$this->CI->db->simple_query('UPDATE ' . $this->user_table  . ' SET last_login = "' . date('c') . '" WHERE id = ' . $user_data['id']);
	...
	*/
	function edit_password($user_email = '', $old_pass = '', $new_pass = '')
	{
		...
		$this->CI->db->select('pass');
		$query = $this->CI->db->get_where($this->user_table, array('email' => $user_email));
		$user_data = $query->row_array();

		$hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);	
		if (!$hasher->CheckPassword($old_pass, $user_data['pass'])){ //old_pass is the same
			return FALSE;
		}
